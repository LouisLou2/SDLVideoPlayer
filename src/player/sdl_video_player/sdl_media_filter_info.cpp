...
std::optional<ErrorDesc> SDLMediaFilterInfo::configAudioFilter(
  ...
  std::array<AVSampleFormat,2> sampleFmtPair = {
    THE_ONLY_ALLOWED_SAMPLE_FMT, AV_SAMPLE_FMT_NONE
  };
  ...
  audioFilterGraph->aresample_swr_opts = av_strdup(swrOptsPairStr.data()); // configureAuduoFilter函数退出后，swrOptsPairStr的内存释放掉， 但是那是好像这个字段也已经作用发挥完了，之后再继续观察学习， 看是不是有问题

  // 在使用bp之前，要初始化
  av_bprint_init(&bp, 0, AV_BPRINT_SIZE_AUTOMATIC);
  av_channel_layout_describe_bprint(&audioFilterParamsSrc.ch_layout, &bp);
  // 准备创建滤镜的参数字符串
  filterArgsStr = std::format(
    "sample_rate={}:sample_fmt={}:time_base={}/{}:channel_layout={}",
    audioFilterParamsSrc.freq,
    av_get_sample_fmt_name(audioFilterParamsSrc.fmt),
    1,
    audioFilterParamsSrc.freq,
    bp.str
  );
  ...
  if (forceOutputFormat) {
    sampleRatePair.front() = audioParamsTarget.freq;
  ...
  audioFilterOut = filterAudioSink;
end:
...
std::optional<ErrorDesc> SDLMediaFilterInfo::configAudioFilterGraph(
  ...
  uint32_t oldNbFilters = audioFilterGraph->nb_filters;
  if (audioFilterGraphStr.empty()) {
    ...
    * 数据流:
     ...
     */
    if ((ret = avfilter_graph_parse_ptr(audioFilterGraph, audioFilterGraphStr.data(), &input, &output, nullptr)) < 0) {
      optErr= ErrorDesc::from(
        ExceptionType::FFmpegSetFailed,
  ...
  // TODO:
  for (uint32_t i = 0; i < audioFilterGraph->nb_filters - oldNbFilters;++i) {
    FFSWAP(AVFilterContext*, audioFilterGraph->filters[i], audioFilterGraph->filters[i + oldNbFilters]);
  }
  // check and link all filters with order
  if ((ret = avfilter_graph_config(audioFilterGraph, nullptr))< 0) {
    optErr = ErrorDesc::from(
      ExceptionType::FFmpegSetFailed,
